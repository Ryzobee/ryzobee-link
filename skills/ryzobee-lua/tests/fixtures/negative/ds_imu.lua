...
local function report_status()
    ...
    if connected == nil then
        print('WiFi: unavailable (' .. tostring(reason) .. ')')
    else
        print('WiFi connected: ' .. tostring(connected))
    end

    local ble_ok, ble_reason = ble.is_connected()
    if ble_ok == nil then
        print('BLE: unavailable (' .. tostring(ble_reason) .. ')')
    else
        print('BLE connected: ' .. tostring(ble_ok))
    end

    local ready, hid_reason = hid.is_ready()
    if ready == nil then
        print('HID: unavailable (' .. tostring(hid_reason) .. ')')
    else
        print('HID ready: ' .. tostring(ready))
    end
end
