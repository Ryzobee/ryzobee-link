...
local function show(handle, red, green, blue)
    ...
    if not ok then
        print('LED write failed:', reason)
        return
    end
    for _ = 1, LED_POLLS do
        local status, err = handle:status()
        if not status then
            print('LED status failed:', err)
            return
        end
...
local function close(handle, label)
    ...
    if not ok then print(label .. ' close failed:', reason) end
end
...
while #received < MAX_RX do
    ...
    local room = MAX_RX - #received
    if room < want then want = room end
    local chunk, info = uart_handle:read(want, READ_SLICE_MS)
    if not chunk then
        read_error = info
        break
    end
...
if #received >= MAX_RX then truncated = true end

print(string.format('UART1 ping sent=%d/%d received=%d loss_possible=%s error_events=%s truncated=%s',
    sent, #PING, #received, tostring(loss_possible), error_events, tostring(truncated)))
if write_error then print('UART write failed:', write_error) end
if read_error then print('UART read failed:', read_error) end
if #received > 0 then
    local hex = {}
...
board.mark('uart_ping_truncated', truncated)

-- Green only means a healthy observed exchange: write completed, bytes arrived,
-- no read/write failure, no reported loss and no truncation. It does not mean a
-- peer acted on the ping.
local sent_ok = (sent == #PING) and (write_error == nil)
local healthy = sent_ok and (read_error == nil) and (#received > 0)
    and (not loss_possible) and (not truncated)
if healthy then
    show(led_handle, 0, 255, 0)      -- green: healthy observed exchange
elseif sent_ok and (read_error == nil) and (#received > 0) then
    show(led_handle, 255, 255, 0)    -- yellow: reply incomplete, loss or truncation
else
    show(led_handle, 255, 0, 0)      -- red: send failed, read failed or no reply
end
