...
- 组织 V1.0.1（fork 预览版 V1.1.0）起静态缓存按 Service Worker scope 区分，只清理自己路径下的旧缓存；AI 页带 query 的离线访问也回到 AI 页。旧版全局前缀缓存不主动清除。草稿数据库仍为 origin 级共享，多个不同路径部署并不等于独立工作区。
